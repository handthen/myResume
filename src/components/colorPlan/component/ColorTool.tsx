    ...
    return (
        <div className={style.tool}>
            <div className="flex jbetween">
                ...
                <div className={style.showBlock} style={{ background: context.store.color, width: ToolConfig.showBlock * 0.85, height: ToolConfig.showBlock * 0.85 }}>
                </div>
            </div>
        </div >
    )
...
function SilderOpacity(): React.ReactElement {
    ...
    useEffect(() => {
        ...
        if (color.indexOf('rgb') != 1) {
            setBarColor(color.replace(`,${store.opacity}`, ''))
        } else {
    ...
    function mousedown(e: MouseEvent) {
        ...
        setStore(state => ({ ...state, color: computColor, opacity: ranger.toFixed(2) }))
        setPosition(state => {
    ...
    function mousemove(e: MouseEvent) {
        ...
        setPosition(state => {
            ...
            requestAnimationFrame(() => {
                ...
                const computColor = computRangerColor(ranger.toFixed(2), storeRefColor.current)
                setStore(state => {
                    return { ...state, color: computColor, opacity: ranger.toFixed(2) }
                })
            })
            return { ...state, x }
        })
    ...
    function computRangerColor(opaction: string, color: string) {
        ...
        if (color.indexOf('rgb') != -1) {
            ...
            computColor = toRgb(color.replace(/^(#)([0-9a-zA-Z]{6}).*$/, "$1$2" + ob16))
        }
        return computColor
    }
    const criceStyle = {
